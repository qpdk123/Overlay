...
namespace Overlay
{
    public partial class MainForm : Form
    {
        ...
        private SolidBrush brush_dot_LawnGreen = new SolidBrush(Color.LawnGreen);
        ...
        {
            // 창 크기 보정
            SetWindowPos(this.hwnd, IntPtr.Zero, 0, 0, 1600, 900, 0x0000);
        ...
        private void dF_Panel1_Paint(object sender, PaintEventArgs e)
        {
            ...
            using (Bitmap ov = new Bitmap(Properties.Resources.mapHD.Width, Properties.Resources.mapHD.Height))
            {
                using (Graphics g = Graphics.FromImage(ov))
                {
                    ...
                    g.FillEllipse(this.brush_dot_LawnGreen, me.X - dotSize / 2, me.Y - dotSize / 2, dotSize, dotSize);
                    for (int n = 0; n < LocationData.Ins.Length; ++n)
                    {
                        ...
                        {
                            g.FillRectangle(this.brush_dot_LawnGreen, dot.X - dotSize / 2, dot.Y - dotSize / 2, dotSize, dotSize);
